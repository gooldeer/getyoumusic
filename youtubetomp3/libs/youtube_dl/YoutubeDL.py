...
class YoutubeDL(object):
    ...
    def extract_info(self, url, download=True, ie_key=None, callback=None, extra_info={}):
        '''
        ...
        for ie in ies:
            ...
            try:
                ...
                return self.process_ie_result(ie_result, download=download, callback=callback)
    ...
    def process_ie_result(self, ie_result, download=True, callback=None, extra_info={}):
        """
        ...
        if result_type == 'video':
            ie_result.update(extra_info)
            return self.process_video_result(ie_result, callback=callback)
            ...
            def _fixup(r):
                ...
                return r
            ie_result['entries'] = [
                self.process_ie_result(_fixup(r), download=download, callback=callback)
    ...
    def process_video_result(self, info_dict, download=True, callback=None):
        ...
        if info_dict['extractor'] in [u'youtube', u'Youku']:
            if download:
                self.process_info(info_dict, callback=callback)
            return info_dict
        ...
        if download:
            ...
            for format in formats_to_download:
                ...
                self.process_info(new_info, callback=callback)
        # We update the info dict with the best quality format (backwards compatibility)
        info_dict.update(formats_to_download[-1])
        return info_dict

    def process_info(self, info_dict, callback=None):
        ...
        if not self.params.get('skip_download', False):
            if self.params.get('nooverwrites', False) and os.path.exists(encodeFilename(filename)):
                ...
                try:
                    success = self.fd._do_download(filename, info_dict, callback=callback)
                except (compat_urllib_error.URLError, compat_http_client.HTTPException, socket.error) as err:
